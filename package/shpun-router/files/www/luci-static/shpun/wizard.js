...
`]),
		...
		// ===== JS-логика мастера (адаптация твоего старого wizard.js) =====
		...
		if (wanSkipBtn) {
			wanSkipBtn.onclick = function() { showStep(2); };
		}
		...
		if (wifiSkipBtn) {
			wifiSkipBtn.onclick = function() { showStep(3); };
		}
		...
		// — опрос состояния (код, QR, VPN)
		function updateState(delay) {
			if (typeof delay === 'number' && delay > 0) {
				setTimeout(function() { updateState(); }, delay);
				return;
			}
